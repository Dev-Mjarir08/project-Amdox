...
export default function AccountsPayable() {
  ...
  const fetchInvoices = async () => {
    try {
      ...
      console.error("Failed to fetch AP invoices:", err);
  ...
  const handleUpdateStatus = async (id, currentStatus) => {
    ...
    if (currentStatus?.toLowerCase() === 'approved') {
      nextStatus = 'paid';
    } else if (currentStatus?.toLowerCase() === 'paid') {
      toast.info('Invoice is already fully paid');
      return;
    }
    
    try {
      await api.put(`/finance/ap/invoices/${id}/status`, { status: nextStatus });
  ...
  const filteredInvoices = invoices.filter(invoice => {
    const matchesSearch = (invoice.invoiceNumber && invoice.invoiceNumber.toLowerCase().includes(searchText.toLowerCase())) ||
                          ((invoice.vendorName || invoice.vendor || '').toLowerCase().includes(searchText.toLowerCase())) ||
  ...
  const totalOutstanding = invoices
    .filter(i => ['pending', 'approved'].includes(i.status?.toLowerCase()))
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl border border-white/70 bg-white/85 p-6 shadow-soft backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">₹{totalOutstanding.toLocaleString()}</p>
            </div>
        ...
        <div className="rounded-xl border border-white/70 bg-white/85 p-6 shadow-soft backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">₹{pendingApproval.toLocaleString()}</p>
            </div>
        ...
        <div className="rounded-xl border border-white/70 bg-white/85 p-6 shadow-soft backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">Due / Approved</p>
              <p className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">₹{dueThisWeek.toLocaleString()}</p>
            </div>
        ...
        <div className="rounded-xl border border-white/70 bg-white/85 p-6 shadow-soft backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">Rejected Value</p>
                ...
                {filteredInvoices.length === 0 ? (
                  ...
                  filteredInvoices.map((invoice) => (
                    <tr key={invoice.id || invoice._id} className="border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                      ...
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{invoice.vendorName || invoice.vendor}</td>
                      ...
                      <td className="px-4 py-3 text-center">
                        {invoice.status?.toLowerCase() !== 'paid' && invoice.status?.toLowerCase() !== 'rejected' && (
                          <button
                            onClick={() => handleUpdateStatus(invoice.id || invoice._id, invoice.status)}
                            title={invoice.status?.toLowerCase() === 'pending' ? 'Approve Invoice' : 'Mark as Paid'}
