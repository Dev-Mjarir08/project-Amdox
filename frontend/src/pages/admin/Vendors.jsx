...
import { FiBriefcase, FiDownload, FiFilter, FiPlus, FiSearch, FiEdit, FiMail, FiPhone, FiMapPin, FiTrash2 } from 'react-icons/fi';
...
export default function Vendors() {
  ...
  const confirmDeleteVendor = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/supply-chain/vendors/${deleteConfirmId}`);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <button onClick={() => setIsModalOpen(true)} className="erp-focus inline-flex h-11 items-center gap-2 rounded-xl bg-primary px-4 text-sm font-bold text-white shadow-lg shadow-blue-600/20 transition hover:bg-blue-700">
        ...
        <div className="rounded-xl border border-white/70 bg-white/85 p-6 shadow-soft backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">{vendors.length}</p>
            </div>
        ...
        <div className="rounded-xl border border-white/70 bg-white/85 p-6 shadow-soft backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/80">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">
                {vendors.filter(v => v.totalSpend > 500000).length}
              </p>
            </div>
                    ...
                    <tr key={vendor.id} className="border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <button onClick={() => handleDeleteVendor(vendor.id)} className="erp-focus inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-rose-600 shadow-sm transition hover:border-rose-400 hover:text-rose-600 dark:border-slate-800 dark:bg-slate-900 dark:text-rose-400" title="Delete Vendor">
